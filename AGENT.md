...
| | | Every `tree` event carries `repairs` — `recovered`, `rebuilt`, `formulas`, `attached` (the text layer read back, recover.py), `joined`, `stitched`, `deduplicated`, `judged` — and `dropped` — `furniture`, `picture`, `label`, `junk` — so what the reader put back or left out is visible per paper |

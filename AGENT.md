...
| *(confidence)* | | Not an op: `confidence.py` scores every reading as it is saved (`papers.confidence`, in the `tree` event as `confidence: {confidence, reasons}`): plain measurements of the tree — the share of the prose in the abstract, in back matter, in the reference list as long paragraphs, in the single largest lane; lanes the paper's type should have and lacks; text said twice; headings that are not headings; paragraphs cut in two — against limits read off 199 papers held as both PDF and XML (`pairs.py`). At 0.9 or more, four readings in five matched their XML well; under 0.5, five in six were seriously off. Say the reasons with the number; a low score is a reading to look at, not a paper to drop. `python -m litrag_parser.pairs --pdf-lib DIR --xml-lib DIR [--json FILE] [--show KEY]` compares the two readings of every paper two libraries share; `python -m litrag_parser.confidence --calibrate FILE…` scores the score |
...
| `papers` | `lib` | `papers`: rows of `papers` with a `nodes` count — among the columns `type`, `subtype` and `type_source` (record · jats · subject · title · printed · shape · default · none), `pub_types`, `authors` (JSON `[{name, affiliations, corresponding}]`), `journal`, `year`, `confidence` (0–1: how far the reading can be trusted, from the reading alone) and `confidence_detail` (JSON `{reasons: [...], penalties: {check: points}}`) |
